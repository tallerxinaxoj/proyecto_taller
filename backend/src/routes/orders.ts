...
import { isDpiGT, isE164, toDashedDpi } from '../utils/validate';
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const id = (req.params as any).id || 'na';
    const name = `order-${id}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${ext}`;
    cb(null, name);
  }
});
const uploader = multer({
  ...
  fileFilter: (_req, file, cb: multer.FileFilterCallback) => {
    ...
    // Rechazar sin propagar Error para evitar problemas de tipos en build
    return cb(null as any, false);
  }
});
...
router.get('/:id/photos', async (req, res) => {
  const id = Number(req.params.id);
  const photos = await prisma.orderPhoto.findMany({ where: { orderId: id }, orderBy: { createdAt: 'desc' } });
  res.json(photos);
});

// Subir fotos a una orden
router.post('/:id/photos', uploader.array('photos', 10), async (req, res) => {
  const id = Number(req.params.id);
  try {
    ...
    res.status(400).json({ error: e.message || 'No se pudieron subir las fotos' });
  }
});
...
router.delete('/:id/photos/:photoId', async (req, res) => {
  ...
  try {
    const p = await prisma.orderPhoto.findUnique({ where: { id: photoId } });
    if (!p || (p.orderId !== id)) return res.status(404).json({ error: 'No encontrada' });
