...
export default function Products() {
  ...
  const create = async (e: React.FormEvent) => {
    ...
    await apiFetch('/api/products', { method: 'POST', body: JSON.stringify({ name: form.name, sku: form.sku, stock, price }) }, token);
      ...
      {user?.role === 'ADMIN' && (
        <form onSubmit={create} className="bg-grayish p-3 rounded grid grid-cols-4 gap-3">
          ...
          <div className="col-span-1">
            ...
            <input id="p-stock" className="bg-dark p-2 rounded w-full" type="text" inputMode="numeric" placeholder="0" value={form.stock}
              onChange={e=>{
                const only = e.target.value.replace(/\D/g,'');
                const cleaned = only.replace(/^0+(?=\d)/,'');
                setForm({...form, stock: cleaned});
              }}
            />
          </div>
          <div className="col-span-1">
            <label htmlFor="p-price" className="block text-xs text-gray-300 mb-1">Precio</label>
            <input id="p-price" className="bg-dark p-2 rounded w-full" type="text" inputMode="decimal" placeholder="0" value={form.price}
              onChange={e=>{
                const filtered = e.target.value.replace(/[^\d.]/g,'');
                const singleDot = filtered.replace(/(\..*)\./g,'$1');
                const cleaned = singleDot.replace(/^0+(?=\d)/,'');
