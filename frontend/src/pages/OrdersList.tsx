...
type Order = {
  ...
  client: { name: string; whatsapp?: string; dpi?: string };
...
export default function OrdersList() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-3">
        <input
          className="bg-dark p-2 rounded w-full max-w-md"
          placeholder="Buscar por código, cliente, placa o DPI"
          value={q}
          onChange={e=>setQ(e.target.value)}
        />
      </div>
      <div className="grid gap-3">
        {orders.filter(o => { const hay = `${o.code} ${(o.client?.name||'')} ${(o.client?.dpi||'')} ${(o.motorcycle?.plate||'')}`.toLowerCase(); return hay.includes(q.toLowerCase()); }).map(o => (
          <div key={o.id} className="bg-grayish p-3 rounded grid gap-3">
            <div className="flex justify-between gap-3 flex-wrap">
              ...
              {user?.role && (
                <div className="flex gap-2 items-center">
                  <select id={`st-${o.id}`} className="bg-dark p-1 rounded" defaultValue={o.status}>
            ...
            <div className="text-xs text-gray-300">DPI: {o.client?.dpi || '-'}</div>
            {/* Items de la orden */}
